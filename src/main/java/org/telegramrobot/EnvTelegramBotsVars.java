package org.telegramrobot;

public interface EnvTelegramBotsVars {
    final String BOTUSERNAME = "InetIstore_bot";
    final String TELEGRAMTOKENAPI = "5679691022:AAFgTNPbEhg6PU3YJpcSlQrrdspQaLZONok";
    final String STARTDESCRIPTION = "Welcome to InetItemStore Telegram bot, where you can buy NFT-s, Game Codes" +
            " and Gift Cards with crypto or credit card.";
    final String PROVIDERTOKEN = "5322214758:TEST:5ae56668-e314-4c19-b0b3-2251928ed04f";

    final String BOTUSERNAMEADMIN = "inetitemstore_adminbot";
    final String TELEGRAMTOKENAPIADMIN = "5856221681:AAFPNoZLsyOi7z9pqm2btJA7Pn0tJR_IXnY";
    final String STARTDESCRIPTIONADMIN = "Welcome to InetItemStore Admin bot, here you can manage your store." +
            "\n" +
            "\n" +
            "/ban_users - Ban user \n" +
            "/unban_users - Unban user \n" +
            "/promote_user - Promote to Operational User \n" +
            "/disable_opsuser - Disable Operational User \n" +
            "/whoami - What type of Ops User are you \n" +
            "/add_item - Add Item to Store \n" +
            "/del_item - Remove Item from Store \n" +
            "/get_webitems - Get list of webitems by type \n" +
            "/del_order - Remove Order from Store \n" +
            "/get_orders - Get list of orders by type \n";

}
